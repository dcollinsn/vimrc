...
function! s:recompile_spelling_files ()
    if !filereadable('~/.vim/grammarian/errors/spell/en.latin1.add.spl')
        exec 'mkspell ~/.vim/grammarian/errors/spell/en.latin1.add'
    endif
    if !filereadable('~/.vim/grammarian/cautions/spell/en.latin1.add.spl')
        exec 'mkspell ~/.vim/grammarian/cautions/spell/en.latin1.add'
    endif
endfunction
...
function! Grammarian_Toggle ()
    ...
    if empty(s:grammarian_query_type)
        ...
        let &runtimepath = substitute(&runtimepath, ',~/.vim/grammarian/\w\+','','g')
    ...
    " Otherwise, start grammar-correcting...
        ...
        let &runtimepath = substitute(&runtimepath, ',~/.vim/grammarian/\w\+','','g')
        let &runtimepath .= ',~/.vim/grammarian/'.s:grammarian_query_type
        let &encoding = &encoding

        " Load explanations...
        exec 'source ~/.vim/grammarian/'.s:grammarian_query_type.'/data.vim'
